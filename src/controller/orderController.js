...
//         return res.status(400).send({status: false,msg:`invaild parameters please provide order details`})

//         if (!validator.isValidObjectId(req.params.userId))
//       return res.status(400).json({ status: false, message: `${req.params.userId} is not a valid user id` });

//     let findUser = await userModel.findOne({ _id: req.params.userId });

//     if (req.userId != req.params.userId)
//       return res.status(401).json({ status: false, msg: "Authorised user to create order" });

//     if (!findUser)
//       return res.status(400).json({ status: false, message: `${req.params.userId} user id does not exists` });

//       let isUserId = await orderModel.findOne({ userId: req.params.userId });

//     if (isUserId)
//       return res.status(400).json({status: false, message: `${req.params.userId} order detail already exists`,});

//       let {items ,totalPrice,totalItems,totalQuantity} = req.body

//       if (!validator.isValid(items))
//       return res.status(400).json({status: false, message: `items fields is required`, });
...
//         return res.status(400).json({status: false, message: `product Id  is not valid id`, });
    
//         var findProduct = await productModel.findOne({ _id: req.body.items.productId, isDeleted: false, });
...
//       return res.status(400).json({status: false, message: `total price is required`,});

//     if (!validator.isValid(totalItems))
//       return res.status(400).json({ status: false, message: `total items is required`, });

//       let newOrder = { userId: req.params.userId,items , totalPrice, totalItems: 1,totalQuantity}
       
//       let createdOrder = await orderModel.create(newOrder)
//       res.status(400).send({ status:true, msg: `order successfully created`, data: createdOrder})
//     } catch (error) {
...
//     let {items ,totalPrice,totalItems,totalQuantity } = requestBody
//     const findOrder = await orderModel.findById({ _id:req.params.orderId , isDeleted:false})
//     if(!findOrder){
//       return res.status(400).send({ status:false,msg:`orderId is not exist`})
//     }
//     const orderDetail ={
//       items:items, totalPrice:totalPrice, totalItems:totalItems, totalQuantity:totalQuantity,status: status}
    

//     let updatedOrder = await orderModel.findOneAndUpdate({ _id:req.params.orderId} , orderDetail, {new :true})
//     res.status(400).send({ status:true , msg: `updated order successful`, data : updatedOrder})
//   } catch (err) {
